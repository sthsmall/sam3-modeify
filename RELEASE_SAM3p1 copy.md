...
SAM 3 的视频管道独立处理每个被跟踪的对象，其处理时间与对象数量成线性比例。Object Multiplex 将对象分组到固定容量的桶中并联合处理它们，大幅减少冗余计算。有关技术详细信息，请参阅 [SAM 3 论文](https://arxiv.org/abs/2511.16719) 中的附录 H（Object Multiplex）。
...
#### 主要改进
- 在单个 H100 GPU 上处理 128 个对象时，与 2025 年 11 月发布的 SAM 3 相比，**速度提升约 7 倍**
- 显著提高多目标跟踪效率的推理优化：
  - 减少检测-跟踪器关联和其他启发式方法中的 CPU-GPU 同步
  - 增强的 `torch.compile` 支持，改进了操作融合
  - 批处理后处理和视觉编码器，提高 GPU 利用率
- 在 SA-Co/VEval 视频基准测试上的结果喜忧参半，在 YT-Temporal-1B 上有显著改进（+2.1 cgF1）
...
<table style="min-width: 80%; border: 2px solid #ddd; border-collapse: collapse">
  <thead>
    <tr>
      ...
      <th colspan="6" style="text-align: center; border-right: 2px solid #ddd; padding: 10px 16px">SA-Co/VEval 基准测试分割</th>
...
- [`sam3.1_video_predictor_example.ipynb`](examples/sam3.1_video_predictor_example.ipynb)：演示如何使用带有 Object Multiplex 的 SAM 3.1 进行视频分割和使用文本和点提示进行密集跟踪。
